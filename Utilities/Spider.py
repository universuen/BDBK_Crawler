...
from Utilities import URL_Manager
...
class Spider:
    def __init__(self, init_url:str, mode:str, dir:str = "./", max_pages:int = 100):
        # 设置爬行模式
        self.url_manager = URL_Manager(mode)
        # 将第一个url放入url管理器
    ...
    def run(self):
        ...
        while not self.url_manager.is_empty():
            ...
            # 从网页中解析出新的url
            raw_urls = soup.find_all('a', target="_blank", href=re.compile(r"/item/(.*?)"), attrs={"data-lemmaid": re.compile(r"(.*?)")})
